...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("""/start – Menu principal\n/add <token_address>\n/delete <token_address>\n/reset\n/settings\n/help""")

telegram_app = ApplicationBuilder().token(TELEGRAM_TOKEN).build()

conv_handler = ConversationHandler(
    entry_points=[CommandHandler("start", start), CallbackQueryHandler(settings, pattern="^settings$")],
    states={SELECTING_SETTING: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_value), CallbackQueryHandler(setting_selected)]},
    fallbacks=[]
)
